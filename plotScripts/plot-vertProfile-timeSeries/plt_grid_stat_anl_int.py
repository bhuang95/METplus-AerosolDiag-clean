...
sa_path="%s/wrk-%s-%s-%s/met_grid_stat_anl/%s-%s-INTEGRAL/met_tool_wrapper/stat_analysis/" %(expdir, modelname, obsname, suffix, modelvar, obsvar) 
sa_path1="%s/wrk-%s-%s-%s/met_grid_stat_anl/%s-%s-INTEGRAL/met_tool_wrapper/stat_analysis/" %(expdir, modelname1, obsname1, suffix, modelvar1, obsvar1) 
...
for nlv in np.arange(nlev):
    ...
    ax.set_ylabel('Total Column Burden [Unit: \u03bcg/m$^2$]',fontsize=14)
    ax.grid()
    ax.set_title('%s %s' %(area,modelvar),loc='center',fontsize=18)

    if (fsave):
        fig.savefig('./TimeSeries_%s.%s_%s_%s_%s.%s_%s.png' %(area,modelvar,obsname1,modelname,obsname,sdate,cdate), format='png')
