...
for irow in range(nrows):
    ...
    if irow == 0:
        ...
        cbarmax=100
        cbarextend='both'
        biasmax=np.amax(np.absolute(pdata1))
        biasmax=biasmax*0.4
        if biasmax==0:
            biasmax=1.0
        vmin=-1.0*biasmax
        ...
        cbarmax=250
        cbarextend='max'
        rmsemax=np.amax(pdata1)
        rmsemax=rmsemax*0.4
        if rmsemax==0:
            rmsemax=1.0
        vmin=0.0 #-1.0*rmsemax
        vmax=rmsemax
        n=40
    ...
    for icol in range(ncols):
        ...
        if irow == 0:
            tname='%s TCB %s %s [\u03bcg/m$^2$]' %(fcstvar,modelname, metname)
        else:
            tname='%s %s TCB  %s agst. MERRA2 [\u03bcg/m$^2$]' %(modelname, fcstvar, metname)
